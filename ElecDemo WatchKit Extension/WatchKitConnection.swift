...
extension WatchKitConnection: WatchKitConnectionProtocol {
    ...
    {
        validReachableSession?.sendMessage(message, replyHandler: nil)
        //AMIT: Send without replyHandler (Dont expect a reply) prevents WCSession reply Timeout from App
//        { (result) in
//            print(result)
